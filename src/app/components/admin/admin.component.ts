...
export class AdminComponent implements OnInit {
  ...
  cargarDatos() {
    ...
    for (let i = 0; i < localStorage.length; i++) {
      ...
      if (key && key.startsWith('registroUsuario')) {
        const data = localStorage.getItem(key);
        if (data) {
          template.push({ key, data: JSON.parse(data) });
        }
      }
    }
    ...
    for (let i = 0; i < localStorage.length; i++) {
      const key = localStorage.key(i);
      if (key && key.startsWith('comentario')) {
        const data = localStorage.getItem(key);
        if (data) {
          comentarios.push({ key, data: JSON.parse(data) });
        }
      }
    }
